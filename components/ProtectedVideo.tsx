  ...
  if (!hasPurchase) {
    ...
    return (
      <div style={centeredCard} className={cardCls}>
        ...
        {/* Path 1: Buy from website */}
        <a
          href={shopUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-block bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg px-6 py-3 text-sm transition-colors"
        >
          Buy Now →
        </a>
        ...
        <div className="flex items-center gap-3 my-5">
          <div className={`flex-1 h-px ${lightMode ? "bg-gray-200" : "bg-white/10"}`} />
          <span className={`text-xs font-medium ${lightMode ? "text-gray-400" : "text-slate-500"}`}>OR</span>
          <div className={`flex-1 h-px ${lightMode ? "bg-gray-200" : "bg-white/10"}`} />
        </div>

        {/* Path 2: Amazon buyers */}
        <div className={`rounded-xl px-4 py-3 mb-4 text-left ${lightMode ? "bg-amber-50 border border-amber-200" : "bg-amber-500/10 border border-amber-500/20"}`}>
          ...
          <p className={`text-xs leading-relaxed ${lightMode ? "text-amber-800" : "text-amber-300/80"}`}>
            Your access needs to be activated manually. Tap the button below and we&apos;ll unlock your videos within minutes.
          </p>
        </div>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={`inline-flex items-center gap-1.5 text-xs font-semibold px-4 py-2.5 rounded-lg border transition-colors ${lightMode ? "text-green-700 border-green-400 hover:bg-green-50" : "text-green-400 border-green-500/50 hover:bg-green-500/10"}`}
        >
        ...
        <br />
        <button onClick={logout} className={`${logoutCls} mt-3`}>
          Logout
        </button>
      </div>
    );
