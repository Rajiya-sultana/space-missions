...
export default function UserProfileButton() {
  const { user, logout, purchaseMap, justLoggedIn } = useAuth();
  const [open, setOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const hasAnyPurchase = Object.values(purchaseMap).some(Boolean);
          ...
          <div className="absolute right-0 top-11 w-60 bg-[#0d1117] border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden">
            ...
            <div className="px-4 pb-3 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-white text-sm font-medium truncate max-w-[140px]">{email}</p>
                  <p className="text-slate-500 text-xs">Explorer</p>
                </div>
              </div>
            </div>

            <div className="px-4 py-3 border-b border-white/5 flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${hasAnyPurchase ? "bg-green-400" : "bg-slate-500"}`} />
              <span className="text-xs text-slate-400">
                {hasAnyPurchase ? "Full access unlocked" : "No purchase found"}
              </span>
            </div>
